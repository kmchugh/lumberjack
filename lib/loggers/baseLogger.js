...
module.exports = function(config){
    ...
    var _defaults = {
        format : {
            ...
            info : '[%event%](%date%) - %message%' + require('os').EOL + '%data%',
    ...
    var createEntry = function(level, event, message, data, err) {
        return {
            ...
            date: new Date(),
            logLevel: level || this.config.defaultLevel,
    ...
    return {
        ...
        decorate: function(logObject) {
            if (typeof logObject === 'object') {
                var self = this;
                logObject._lumberjack=self;
                ['info', 'debug', 'warning', 'error'].map(function(level){
                    logObject[level] = function(){
                        self.log.apply(self, [level.toUpperCase()].concat(Array.prototype.slice.call(arguments, 0)));
                    };
                });
            }
        },
